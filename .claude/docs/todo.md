# TODO List

이 문서는 프로젝트에서 구현해야 할 기능들을 추적합니다.

---

## [완료] Feature 1: 로그인/회원가입 기능

### DB (db-agent)
- [x] User 모델 설계 (`backend/app/models/user.py`)
- [x] User CRUD 함수 (`backend/app/crud/user.py`)

### BE (be-agent)
- [x] Pydantic 스키마 (`backend/app/schemas/auth.py`)
- [x] 인증 유틸리티 (`backend/app/utils/auth.py`)
- [x] 인증 API 엔드포인트 (`backend/app/routers/auth.py`)
- [x] 의존성 설치 (python-jose, passlib, python-multipart)
- [x] 환경변수 설정 (.env)

### FE (fe-agent)
- [x] TypeScript 타입 정의 (`frontend/src/types/auth.ts`)
- [x] API 호출 함수 (`frontend/src/lib/api/auth.ts`)
- [x] 인증 Context (`frontend/src/contexts/AuthContext.tsx`)
- [x] 로그인 페이지 (`frontend/src/app/login/page.tsx`)
- [x] 회원가입 페이지 (`frontend/src/app/register/page.tsx`)
- [x] 대시보드 페이지 (`frontend/src/app/dashboard/page.tsx`)
- [x] 메인 페이지 로그인 리다이렉트

---

## [완료] Feature 2: 주식 현황 기능

코스피 시총 TOP 10 종목의 현재가격을 테이블 형태로 보여주는 기능

### BE (be-agent)

#### 2.1 주식 API 엔드포인트
- [x] 주식 라우터 작성 (`backend/app/routers/stocks.py`)
  - [x] `GET /api/stocks/kospi-top10` - 코스피 시총 TOP 10 현재가 조회
- [x] 주식 스키마 작성 (`backend/app/schemas/stock.py`)
  - [x] `StockInfo` - 종목 정보 (종목코드, 종목명, 현재가, 등락률 등)
  - [x] `StockListResponse` - 종목 리스트 응답

#### 2.2 외부 API 연동
- [x] 주식 데이터 서비스 작성 (`backend/app/services/stock.py`)
  - [x] pykrx 라이브러리 활용
  - [x] 코스피 시총 TOP 10 종목 데이터 조회

#### 2.3 의존성 설치
- [x] `requirements.txt`에 의존성 추가
  - [x] `pykrx` (주식 데이터)

### FE (fe-agent)

#### 2.4 주식 관련 타입 정의
- [x] TypeScript 인터페이스 작성 (`frontend/src/types/stock.ts`)
  - [x] `Stock` - 종목 정보
  - [x] `StockListResponse` - 종목 리스트 응답

#### 2.5 API 호출 함수
- [x] 주식 API 함수 작성 (`frontend/src/lib/api/stock.ts`)
  - [x] `getKospiTop10()` - 코스피 TOP 10 조회

#### 2.6 주식 현황 페이지
- [x] 주식 현황 페이지 작성 (`frontend/src/app/stocks/page.tsx`)
  - [x] 페이지 제목 (주식 현황)
  - [x] 코스피 시총 TOP 10 테이블
    - [x] 순위
    - [x] 종목코드
    - [x] 종목명
    - [x] 현재가
    - [x] 전일대비 (등락률)
    - [x] 거래량
  - [x] 로딩 상태 표시
  - [x] 에러 처리

#### 2.7 네비게이션 메뉴 추가
- [x] Header 컴포넌트에 "주식 현황" 메뉴 추가 (`frontend/src/components/Header.tsx`)
- [x] 로그인한 사용자만 접근 가능하도록 설정

---

## Feature 3: 게임 탭 (오목)

게임 메뉴를 추가하고, 오목 게임을 플레이할 수 있는 기능

### FE (fe-agent)

#### 3.1 게임 페이지 라우팅
- [ ] 게임 메인 페이지 (`frontend/src/app/games/page.tsx`)
  - [ ] 게임 목록 표시 (현재: 오목)
  - [ ] 각 게임으로 이동하는 카드/버튼

#### 3.2 오목 게임 구현
- [ ] 오목 게임 페이지 (`frontend/src/app/games/omok/page.tsx`)
  - [ ] 15x15 바둑판 렌더링
  - [ ] 흑/백 돌 번갈아 놓기
  - [ ] 승리 조건 판정 (5개 연속)
  - [ ] 게임 상태 표시 (현재 차례, 승자)
  - [ ] 게임 리셋 기능
- [ ] 오목 게임 컴포넌트 (`frontend/src/components/games/OmokBoard.tsx`)
  - [ ] 바둑판 그리드 UI
  - [ ] 돌 놓기 인터랙션
  - [ ] 마지막 수 하이라이트
- [ ] 오목 게임 로직 (`frontend/src/lib/games/omok.ts`)
  - [ ] 승리 판정 알고리즘
  - [ ] 게임 상태 관리 타입

#### 3.3 게임 관련 타입 정의
- [ ] TypeScript 인터페이스 (`frontend/src/types/game.ts`)
  - [ ] `OmokState` - 오목 게임 상태
  - [ ] `Player` - 플레이어 (흑/백)
  - [ ] `BoardPosition` - 바둑판 좌표

#### 3.4 네비게이션 메뉴 추가
- [ ] Header 컴포넌트에 "게임" 메뉴 추가
- [ ] 로그인한 사용자만 접근 가능하도록 설정

---

## 작업 순서 (권장)

### Feature 3: 게임 탭 (오목)
1. **FE**: 게임 관련 타입 정의 (3.3)
2. **FE**: 오목 게임 로직 (3.2)
3. **FE**: 오목 게임 컴포넌트 (3.2)
4. **FE**: 게임 페이지 (3.1, 3.2)
5. **FE**: 네비게이션 메뉴 추가 (3.4)

---

## 참고사항

### 코스피 시총 TOP 10 종목 (2024년 기준)
1. 삼성전자 (005930)
2. SK하이닉스 (000660)
3. LG에너지솔루션 (373220)
4. 삼성바이오로직스 (207940)
5. 현대차 (005380)
6. 기아 (000270)
7. 셀트리온 (068270)
8. KB금융 (105560)
9. 신한지주 (055550)
10. POSCO홀딩스 (005490)

### 주식 데이터 소스 옵션
- **pykrx**: 한국거래소 데이터 (무료, 간편)
- **FinanceDataReader**: 다양한 금융 데이터 (무료)
- **한국투자증권 OpenAPI**: 실시간 데이터 (API 키 필요)
- **공공데이터포털**: 금융위원회 API (API 키 필요)
